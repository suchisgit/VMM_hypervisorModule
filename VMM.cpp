...
class VirtualMachine{
	...
	VirtualMachine(string path,vector<int> previousRegisters, int instructionNumber){
		vm_binary=path;
		// memset(registers,0,sizeof(registers));
		for(int i=0;i<32;i++){
			registers[i]=previousRegisters[i];
		}
		hi=0;
		low=0;
		currInstruction = instructionNumber;
		instructions=getAllInstructions(vm_binary);
	}
	...
	int runMIPSinstructions(string instruction){
	  ...
	  //Logical operations.
		...
		else if (instruction_l=="MIGRATE"){
			string ipOrHost = instruction_r ;
			// need to pass these variables to next object and start from there
			// cout << "Remote Machine (" + ipOrHost + ") :"  << endl ;
			return 2;
		}
		currInstruction++;
		return 0;
...
class VirtualMachineMonitor  : public VirtualMachine {
	...
	void triggerExecutionWithContextSwitch(vector<int> order){
		// string dummypath="dummy";
		// cout << "here myann";
		vector<int> dummyvec(32,1);
		int output;
		VirtualMachine VM3=VirtualMachine(VM1.vm_binary,dummyvec,0);
		for ( int i=0;i<order.size();i++ ){
			if ( order[i] == 1){
				...
				// cout << "here myann 2";
				output=VM1.runMIPSinstructions(VM1.instructions[VM1.currInstruction]);
				// VirtualMachine(string path,vector<int> previousRegisters, int instructionNumber)
				// cout << "output" << output << VM1.currInstruction << endl;
				if(output == 2){
					// cout << "here myann 3";
					...
					// cout << "trigger new vm with already present values" << endl;
					// cout << VM1.registers[0] << endl;
					// cout << VM1.registers[1] << endl;
					vector<int> regs;
					for(int i=0;i<32;i++){
			       regs.push_back(VM1.registers[i]);
			    }
					// VM3=VirtualMachine(VM1.vm_binary,regs,VM1.currInstruction);
					// createThirdVM(VM1.vm_binary,regs,VM1.currInstruction);
					// VM3.vm_binary="instructions_m";
					// VM3.vm_binary=VM1.vm_binary;
					// cout << "here myann 4";
					for(int i=0;i<32;i++){
						VM3.registers[i]=regs[i];
					}
					VM3.currInstruction=VM1.currInstruction+1;
					// cout << "here myann 5";
					// break;
				...
				//migration state
				// cout << "here myann 10" << endl;
				// cout << VM3.instructions[3] << endl;
				if (VM3.instructions[VM3.currInstruction]=="DUMP_PROCESSOR_STATE"){
					cout << "-----" << endl;
					cout << "Remote Macchine:" << endl;
				}
				VM3.runMIPSinstructions(VM3.instructions[VM3.currInstruction]);
				// cout << VM3.currInstruction << endl;
			}
		}
	}
